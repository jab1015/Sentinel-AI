...
namespace Sentinel.App
{
    public sealed partial class MainWindow : Window
    {
        ...
        private readonly ApprovedRemediationDispatcher _approvedRemediationDispatcher = new();
        ...
        private async Task ReviewApprovedRemediationAsync() { SubscriptionState subscription = await _subscriptionService.GetStateAsync(); if (!subscription.IsActive) { ContentDialog required = new() { Title = "Subscription required", Content = "Free local monitoring and investigation remain active. An active Sentinel AI subscription is required before Sentinel can perform a repair or containment action.", CloseButtonText = "OK", XamlRoot = ((FrameworkElement)Content).XamlRoot }; await required.ShowAsync(); return; } var snapshot = _engine.CurrentSnapshot; RemediationApprovalCoordinator.RemediationApprovalRequest? request = _approvalCoordinator.CreateRequest(snapshot); if (request is null) return; string targetDescription = request.Action.Equals("block-outbound-endpoint", StringComparison.OrdinalIgnoreCase) ? $"Flagged connection: {request.Target}\nFirewall scope: all outbound traffic to the remote IP represented by this connection, across applications, services, profiles, protocols, and ports." : $"Target: {request.Target}"; ContentDialog dialog = new() { Title = request.Title, Content = $"{request.Summary}\n\n{targetDescription}\n\nSentinel will revalidate the investigation immediately before acting. This approval is single-use and expires automatically.", PrimaryButtonText = "Approve", CloseButtonText = "Cancel", DefaultButton = ContentDialogButton.Close, XamlRoot = ((FrameworkElement)Content).XamlRoot }; ContentDialogResult result = await dialog.ShowAsync(); if (result != ContentDialogResult.Primary) return; await _engine.RefreshForRemediationAsync(); var currentSnapshot = _engine.CurrentSnapshot; RemediationApprovalCoordinator.ApprovalValidationResult validation = _approvalCoordinator.Validate(request, currentSnapshot, true); var execution = await _approvedRemediationDispatcher.ExecuteAsync(
                currentSnapshot,
                request,
                validation,
