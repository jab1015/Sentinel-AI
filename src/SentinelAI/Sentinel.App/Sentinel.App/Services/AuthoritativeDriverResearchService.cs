...
namespace Sentinel.App.Services
{
    ...
    public sealed class AuthoritativeDriverResearchService
    {
        ...
        private static DriverResearchResult Research(string deviceName)
        {
            ...
            if (catalog.Reached && CatalogAppearsToHaveResults(catalog.Body))
            {
                return new DriverResearchResult(
                    ...
                    context.HardwareId,
                    "Windows Update did not offer this repair automatically, but Sentinel found candidate driver information in Microsoft's official Update Catalog for the affected device. Sentinel has not selected or installed a package because the exact catalog entry must still be matched to this computer's hardware ID and verified before installation.",
                    false);
            }
            ...
            if (!string.IsNullOrWhiteSpace(oem.Uri))
            {
                ...
                if (vendor.Reached)
                {
                    return new DriverResearchResult(
                        ...
                        context.SerialNumber,
                        context.HardwareId,
                        $"Windows Update did not offer a compatible driver. Sentinel verified the computer as {Display(context.Manufacturer)} {Display(context.Model)} and reached the manufacturer's official driver-support source. The safest next repair path is the OEM driver for this exact computer. Sentinel will not download or install an unverified package automatically.",
                        true);
                }
            }
            ...
            if (guidance.Reached)
            {
                return new DriverResearchResult(
                    ...
                    context.SerialNumber,
                    context.HardwareId,
                    "Sentinel verified Microsoft's official guidance for Device Manager Code 10. The device failed to start; Microsoft recommends repairing the device driver. Windows Update did not provide an automatic package, so an OEM-specific driver is required before Sentinel can safely perform the repair.",
                    true);
            }
        ...
        private static OemSource BuildOemSource(DeviceContext context)
        {
            ...
            return new("Intel Download Center", "https://www.intel.com/content/www/us/en/download-center/home.html");
        }
        ...
        private static string Display(string value) => string.IsNullOrWhiteSpace(value) ? "this" : value.Trim();

        public sealed record DriverResearchResult(
            bool Completed,
            bool AuthoritativeSourceReached,
