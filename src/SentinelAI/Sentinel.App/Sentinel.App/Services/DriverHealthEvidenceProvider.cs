    ...
    public sealed class DriverHealthEvidenceProvider
    {
        ...
        public DriverHealthSnapshot GetSnapshot()
        {
            string value = RunPowerShell(
                ...
                "$recent=@(Get-WinEvent -FilterHashtable @{LogName='System';StartTime=(Get-Date).AddDays(-7)} -ErrorAction SilentlyContinue | Where-Object {$_.ProviderName -match 'Kernel-PnP|DriverFrameworks|UserPnp'} | Select-Object -First 5); " +
            ...
            if (string.IsNullOrWhiteSpace(value)) return DriverHealthSnapshot.Unavailable();
            ...
            return new DriverHealthSnapshot(true, evidence.Devices, evidence.ProblemDevices, evidence.UnsignedDrivers,
                evidence.RecentDriverEvents, primaryProblem, deviceName, evidence.ProblemDevices > 0);
        }
        ...
        private static string BuildUserFacingResponse(DriverHealthSnapshot evidence)
        {
            ...
            response.AppendLine(string.IsNullOrWhiteSpace(evidence.PrimaryProblem) ? "Windows reported a driver problem, but did not provide a device name." : evidence.PrimaryProblem);
            response.AppendLine();
            response.AppendLine("What this means");
            response.AppendLine(evidence.RecentDriverEventCount == 0 ? "No recent driver-related failures were found in the Windows System log. The device status still needs review." : $"Windows recorded {evidence.RecentDriverEventCount} recent driver-related event{(evidence.RecentDriverEventCount == 1 ? string.Empty : "s")}, so Sentinel should investigate before making a change.");
            response.AppendLine();
            response.AppendLine("Recommended action");
        ...
        {
            int devices = 0, problemDevices = 0, unsignedDrivers = 0, recentDriverEvents = 0;
            string problems = string.Empty, events = string.Empty;
        ...
        private static string RunPowerShell(string command)
        {
            try
            {
                ...
                ProcessExecutionResult result = BoundedProcessRunner.RunAsync(startInfo, CommandTimeout).GetAwaiter().GetResult();
        ...
        public sealed record DriverHealthSnapshot(bool Available, int DevicesChecked, int ProblemDeviceCount,
            int UnsignedDriverCount, int RecentDriverEventCount, string PrimaryProblem, string PrimaryDeviceName, bool RequiresAttention)
        {
            public static DriverHealthSnapshot Unavailable() => new(false, 0, 0, 0, 0, string.Empty, string.Empty, false);
        }

        private sealed record DriverEvidence(int Devices, int ProblemDevices, int UnsignedDrivers,
            int RecentDriverEvents, string Problems, string Events);
    }
}
