    ...
    public class MonitoringEngine
    {
        ...
        public async Task RefreshAsync()
        {
            ...
            SystemSnapshot snapshot = new()
            {
                ...
                FlaggedProcessCount = _processSnapshot.FlaggedProcessCount, PrimaryFlaggedProcessName = _processSnapshot.PrimaryProcessName, PrimaryFlaggedProcessReason = _processSnapshot.PrimaryReason,
                ...
                ReviewedCommandLineProcessCount = _commandLineSnapshot.ReviewedProcessCount, FlaggedCommandLineCount = _commandLineSnapshot.ReviewFindingCount,
                PrimaryCommandLineProcessName = _commandLineSnapshot.PrimaryProcessName, PrimaryCommandLineReason = _commandLineSnapshot.PrimaryReason, PrimaryCommandLineSummary = _commandLineSnapshot.PrimaryCommandLineSummary,
                InstalledServiceCount = _serviceSnapshot.InstalledServiceCount, RunningServiceCount = _serviceSnapshot.RunningServiceCount, FlaggedServiceCount = _serviceSnapshot.FlaggedServiceCount,
                PrimaryFlaggedServiceName = _serviceSnapshot.PrimaryServiceName, PrimaryFlaggedServiceReason = _serviceSnapshot.PrimaryReason,
                ...
                EstablishedConnectionCount = _activeConnectionSnapshot.EstablishedConnectionCount, ExternalConnectionCount = _activeConnectionSnapshot.ExternalConnectionCount, FlaggedConnectionCount = _activeConnectionSnapshot.ReviewConnectionCount,
            ...
            snapshot.RiskScore = assessment.Score; snapshot.RiskLevel = assessment.Level; snapshot.RiskSummary = assessment.Summary; snapshot.Recommendation = assessment.Recommendation;

            GuidanceEngine.GuidanceResult guidance = _guidanceEngine.Analyze(snapshot);
            snapshot.GuidanceTitle = guidance.Title; snapshot.GuidanceSeverity = guidance.Severity; snapshot.GuidanceConfidencePercent = guidance.ConfidencePercent;
            snapshot.GuidanceConfidenceLabel = guidance.ConfidenceLabel; snapshot.GuidanceEvidence = guidance.Evidence; snapshot.GuidanceWhatHappened = guidance.WhatHappened;
            snapshot.GuidanceWhyItMatters = guidance.WhyItMatters; snapshot.GuidanceRecommendedAction = guidance.RecommendedAction; snapshot.GuidanceFixAvailability = guidance.FixAvailability;
            snapshot.GuidanceFixDetails = guidance.FixDetails; snapshot.GuidanceActionId = guidance.ActionId; snapshot.GuidanceActionLabel = guidance.ActionLabel;
        ...
        public async Task<VerificationResult> VerifyCurrentGuidanceAsync()
        {
            ...
            _lastServiceRefresh = DateTime.MinValue; _lastEventLogRefresh = DateTime.MinValue; await RefreshAsync();
        ...
        private static void SuppressNonActionableStorageSpacesSmp(SystemSnapshot snapshot)
        {
            ...
            snapshot.CriticalEventCount = 0; snapshot.ErrorEventCount = 0; snapshot.LatestEventTime = null; snapshot.LatestEventSource = "None"; snapshot.LatestEventMessage = "No actionable Windows events were detected.";
            if (serviceMatch) { snapshot.FlaggedServiceCount = 0; snapshot.PrimaryFlaggedServiceName = "None"; snapshot.PrimaryFlaggedServiceReason = "No actionable service conditions were detected."; }
        }
        ...
        private static void SuppressTransientWindowsUpdateFileInUse(SystemSnapshot snapshot)
        {
            ...
            if (!match) return;
            snapshot.CriticalEventCount = 0; snapshot.ErrorEventCount = 0; snapshot.LatestEventTime = null; snapshot.LatestEventSource = "None";
        ...
        private static string ExtractServiceDisplayName(string message)
        {
            ...
            return start >= 0 && end > start + prefix.Length ? message.Substring(start + prefix.Length, end - start - prefix.Length).Trim() : string.Empty;
        }
        ...
        public TimeSpan Uptime => _windowsInfoMonitor.GetUptime();

        public sealed record VerificationResult(string Title, string Message, bool Resolved);
    }
}
