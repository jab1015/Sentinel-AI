...
Require(duplicateResult.QueryValid && duplicateResult.Exists && !duplicateResult.IsExactBlock, "Duplicate deterministic firewall rules did not fail closed.");
FirewallRuleVerification notFound = FirewallRuleVerificationPolicy.Evaluate(new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["FOUND"] = "0" }, testIp);
Require(notFound.QueryValid && !notFound.Exists, "Explicit firewall rule absence was not classified correctly.");
FirewallRuleVerification malformed = FirewallRuleVerificationPolicy.Evaluate(new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase), testIp);
Require(!malformed.QueryValid, "Malformed firewall query output was incorrectly treated as verified absence.");
FirewallRuleVerification malformedCount = FirewallRuleVerificationPolicy.Evaluate(new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { ["FOUND"] = "not-a-number" }, testIp);
...
Require(incompleteResult.QueryValid && incompleteResult.Exists && !incompleteResult.IsExactBlock, "Incomplete firewall scope evidence was incorrectly accepted.");

string[] approvedServices = { "BITS", "bits", "wuauserv", "Spooler" };
foreach (string service in approvedServices)
    Require(ServiceRestartEngine.TryNormalizeServiceName(service, out string normalized) && !string.IsNullOrWhiteSpace(normalized), $"Allowlisted service was rejected: {service}");

string[] rejectedServices =
{
    "WinDefend", "WdNisSvc", "MpsSvc", "SecurityHealthService", "EventLog", "RpcSs",
    "BITS & net user attacker /add", "../BITS", "*", "", "   "
};
...
string brokerTransactionRoot = Path.Combine(Path.GetTempPath(), "SentinelServiceRestartHarness", Guid.NewGuid().ToString("N"), "Transactions");
Directory.CreateDirectory(brokerTransactionRoot);
try
{
    ServiceRestartEngine emptyRecovery = new(brokerTransactionRoot);
    Require(emptyRecovery.RecoverPending().Succeeded, "Empty service-restart recovery directory should be safe.");

    string quarantineLikeRecord = Path.Combine(brokerTransactionRoot, Guid.NewGuid().ToString("N") + ".json");
    File.WriteAllText(quarantineLikeRecord, "{}");
    Require(emptyRecovery.RecoverPending().Succeeded, "Top-level quarantine transaction records must not be consumed by service-restart recovery.");

    string serviceRestartRoot = Path.Combine(brokerTransactionRoot, "ServiceRestarts");
    Directory.CreateDirectory(serviceRestartRoot);
    string malformedRecord = Path.Combine(serviceRestartRoot, $"service-restart-{Guid.NewGuid():N}.json");
    File.WriteAllText(malformedRecord, "{}");
    ServiceRestartResult malformedRecovery = emptyRecovery.RecoverPending();
    Require(!malformedRecovery.Succeeded && malformedRecovery.Code == "RecoveryRecordInvalid", "Malformed service restart recovery state did not fail closed.");

    Require(Directory.EnumerateFiles(brokerTransactionRoot, "service-restart-*.json", SearchOption.TopDirectoryOnly).Count() == 0,
        "Service restart recovery records must never share the top-level quarantine transaction namespace.");
}
finally
{
    try
    {
        string harnessRoot = Directory.GetParent(brokerTransactionRoot)?.FullName ?? brokerTransactionRoot;
        if (Directory.Exists(harnessRoot)) Directory.Delete(harnessRoot, true);
    }
    catch { }
}
...
Console.WriteLine("Service/quarantine recovery namespaces isolated: PASS");
